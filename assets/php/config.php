<?php
  ...
  function verify_csrf($t) { if (!isset($_SESSION['csrf']) || !hash_equals($_SESSION['csrf'], (string)$t)) { throw new Exception('Invalid CSRF token'); } }
  ...
  function log_activity(PDO $pdo, string $type, ?int $entity_id, string $action, string $entity_title, $details = null): void {
    ...
    $sql = 'INSERT INTO activity_log (actor, entity_type, entity_id, entity_title, action, details) VALUES (?,?,?,?,?,?)';
